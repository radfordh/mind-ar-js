...
AFRAME.registerSystem('mindar-image-system', {
  ...
  _resize: function() {
    ...
    // Get the actual rendered video dimensions from the computed style
    const videoRect = video.getBoundingClientRect();
    const vw = videoRect.width;
    const vh = videoRect.height;

    // Calculate video center position relative to container
    const videoCenterX = videoRect.left + vw / 2 - container.getBoundingClientRect().left;
    const videoCenterY = videoRect.top + vh / 2 - container.getBoundingClientRect().top;

    const proj = this.controller.getProjectionMatrix();

    // Calculate the effective display size that would make video fill container
    const videoRatio = video.videoWidth / video.videoHeight;
    const containerRatio = container.clientWidth / container.clientHeight;
    let effectiveWidth, effectiveHeight;
    if (videoRatio > containerRatio) {
      effectiveHeight = container.clientHeight;
      effectiveWidth = effectiveHeight * videoRatio;
    } else {
      effectiveWidth = container.clientWidth;
      effectiveHeight = effectiveWidth / videoRatio;
    }

    const fov = 2 * Math.atan(1/proj[5] / effectiveHeight * container.clientHeight) * 180 / Math.PI;
    ...
    // Set camera aspect based on container (fullscreen canvas)
    ...
    // Calculate scale and offset to map video space to container space
    const scaleX = vw / effectiveWidth;
    const scaleY = vh / effectiveHeight;

    const offsetX = (videoCenterX - container.clientWidth / 2) / (effectiveWidth / 2);
    const offsetY = -(videoCenterY - container.clientHeight / 2) / (effectiveHeight / 2);

    // Apply offset and scale to projection matrix
    const pm = camera.projectionMatrix;
    pm.elements[0] *= scaleX; // scale horizontal
    pm.elements[5] *= scaleY; // scale vertical
    pm.elements[8] = offsetX * scaleX; // horizontal offset
    pm.elements[9] = offsetY * scaleY; // vertical offset

    // Don't let the resize function override video positioning
    // The CSS will handle video positioning
  }
});
